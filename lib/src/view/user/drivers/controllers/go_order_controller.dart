...
import '../is_order_view_fnish.dart';

class GoOrderController extends GetxController {
  ...
  changeOrderDeliveryStatus() async {
    ...
    request.postAuth().then((value) async {
      Get.back();
      if (value['status']) {
        Get.to(()=> IsOrderViewFnish(order: value['data'],),arguments: [value['data']]);
      }else{
  ...
  @override
  void onInit() {
    createRecord(double.parse(Get.arguments[0]['from_latitude'].toString()),double.parse(Get.arguments[0]['from_longitude'].toString()));
    orderId.value = Get.arguments[0]['id'];
    userLat.value = General.latitude;
    userLong.value = General.longitude;
    deliveryLat.value = double.parse(Get.arguments[0]['from_latitude'].toString());
    deliveryLong.value = double.parse(Get.arguments[0]['from_longitude'].toString());
    originLatitude.value = General.latitude;
    originLongitude.value =  General.longitude;
    destLatitude.value =  double.parse(Get.arguments[0]['from_latitude'].toString());
    destLongitude.value =  double.parse(Get.arguments[0]['from_longitude'].toString());
    addMarker(LatLng(General.latitude, General.longitude),"origin",1,);
    addMarker(LatLng(double.parse(Get.arguments[0]['from_latitude'].toString()), double.parse(Get.arguments[0]['from_longitude'].toString())),"destination",3,);
  ...
  addMarker(LatLng position, String id, typeMarker) async{
    ...
    if(typeMarker == 1){
      ...
      path: ("assets/marker.png"), width: 55);
  ...
  void getCurrentLocation() async {
    ...
    location.onLocationChanged.listen(
      (newLoc) {
        ...
        calculateDistance2(General.latitude,General.longitude,newLoc.latitude,newLoc.longitude);
      },
    );
  }
  ...
  Future<double> calculateDistance2(lat1, lon1, lat2, lon2)async{
    ...
    if(distanceInMeters > 10){
      General().setlatitude(lat2);
      General().setlongitude(lon2);
      General().getlatitude();
      General().getlongitude();
      General().getUserData().then((val){
        updateData(lat2,lon2);
        mapController2.animateCamera( 
          CameraUpdate.newCameraPosition(
            CameraPosition(target: LatLng(lat2,lon2), zoom: 14) 
          )
        );
      });
    }
  ...
  var keyOrderRecord = ''.obs;

  void createRecord(lat,long) async{
    DatabaseReference rootRef = FirebaseDatabase.instance.reference(); 
    var newkey =  rootRef.child('orders').push().key;
    keyOrderRecord.value = newkey!;
    rootRef.push().set({
      'order_id': orderId.value,
      'lat': lat,
      'long': long,
  ...
  void updateData(lat,long) {
    DatabaseReference databaseReference = FirebaseDatabase.instance.reference().child('orders');
    databaseReference.child(keyOrderRecord.value).update({
      'order_id': orderId.value,
      'lat': lat,
      'long': long,
    });
    editUserData(lat,long);
  }

  void editUserData(lat,long) async {
    var data = {
      'latitude': lat,
      'longitude': long,
      'order_id': orderId.value,
    };
    Request request = Request(url: urlTrakOrder, body: data);
