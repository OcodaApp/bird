...
import 'package:birdandroid/src/view/user/drivers/is_order_view_wait.dart';
...
class IsOrderController extends GetxController {
    ...
    request.postAuth().then((value) async {
      ...
      if (value['status']) {
        ...
        General().getUserData().then((val){
          Get.to(()=> IsOrderViewWait(order: value['data'],),arguments: [value['data']]);
        });
  ...
  void onInit() {
      ...
      //   LatLng(General.latitude, General.longitude),
      //   LatLng(double.parse(Get.arguments[0]['from_latitude'].toString()), double.parse(Get.arguments[0]['from_longitude'].toString())),
      // ];
